...
def read_rating(path,data_name, num_users, num_items,num_total_ratings, a, b, test_fold,random_seed,consider_not_voting):
    ...
    if (data_name == 'politic_new') or (data_name == 'politic_old'):
        num_train_ratings = 0
        num_test_ratings = 0
        train_file_name = 'Train_ratings_fold_' + str(test_fold)#'not_voted_roll_call/Train_ratings_fold_' + str(test_fold)
        test_file_name = 'Test_ratings_fold_' + str(test_fold)#'not_voted_roll_call/Test_ratings_fold_' + str(test_fold)
...
def read_trust(path,data_name, num_users):
    if (data_name == 'politic_new') or (data_name == 'politic_old'):
        T = np.load(path + "user_user_matrix.npy") #"not_voted_roll_call/user_user_matrix.npy"
    else:
        raise NotImplementedError("ERROR")
    return T

def read_bill_term(path,data_name,num_items,num_voca):
    file = path + 'Bill_Term' # 'not_voted_roll_call/Bill_Term'
